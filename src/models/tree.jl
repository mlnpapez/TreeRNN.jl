...
mutable struct Tree{C, T, P} <: AbstractTree{C}
    cell::Union{C, Chain{Tuple{C}}}
    children::T
    prob_layer::Union{Dense, Nothing}  # for array nodes only

    # Inner constructor
    Tree(cell::Union{C, Chain{Tuple{C}}}, children::T,prob_layer::P) where {C, T, P} = new{C, T, P}(cell, children, prob_layer) 
end
Flux.@functor Tree
function Tree(m, x::AbstractProductNode, hidden_size::Int, nh::Int=5)
    return Tree(m, map(x->Tree(m, x, hidden_size, nh), x.data), nothing)
end
function Tree(m, x::AbstractBagNode, hidden_size::Int, nh::Int=5)
    return Tree(m, Tree(m, x.data, hidden_size, nh), nothing)
end
function Tree(m, x::ArrayNode, hidden_size::Int, nh::Int=5)
    out_dim = x.data isa OneHotArray ? size(x.data, 1) : 2
    prob_layer = Dense(hidden_size => out_dim)
    return Tree(m, _make_imputing(x.data, Dense(size(x.data, 1) => nh)), prob_layer)
end
...
function (m::Tree)(x::AbstractProductNode, seq_model)
    if Mill.numobs(values(x.data)) == 0
        ...
        # Now results[1] is like:
        # (element = 5×batch, type_bond = 5×batch, ...)

        emb = first.(values(results)) |> state |> m.cell  # Direct processing
...
function (m::Tree)(x::AbstractBagNode, seq_model)
    ...
    joint_log_probs = aggregate_log_probs(child_log_probs, x.bags)
...
function (m::TreeRecur{<:OneStateCell})(x::AbstractMillNode)
    # Reset and resize state for current batch size (e.g. validation set obs)
    batch_size = numobs(x)  # Get number of current observations
    hidden_size = size(m.seq_model.state, 1)
    m.seq_model.state = zeros(Float32, hidden_size, batch_size)

    # Process with correctly sized state
    return m.tree(x, m.seq_model) # Pass seq_model to tree forward pass
end
